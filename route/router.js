...
var BlogIndex = require('../app/controllers/blog/index'),
  // 博客首页模块路由控制器
  Blog = require('../app/controllers/blog/blog'), // 电影模块路由控制器
  BlogComment = require('../app/controllers/blog/blog_comment'), // 电影评论控制器
  BlogCategory = require('../app/controllers/blog/blog_category'), // 电影分类控制器

	// 电影首页控制器
  MovieIndex = require('../app/controllers/movie/movie_index'),
...
module.exports = function (app) {
  ...
  app.route('/admin/user/list')
    ...
    .delete(User.del)
  app.get('/admin/tools',User.signinRequired, User.tools);

  /*============== 博客路由 ==============*/
  // 首页路由
  app.get('/', BlogIndex.index) //ok
  app.get('/blog',BlogIndex.blog) 
  app.get('/blog/results', BlogIndex.search)
  //博客路由
  app.get('/blog/tag', Blog.tag) //ok
  app.get('/blog/:id', Blog.detail)  //ok
  app.get('/admin/blog/new', User.signinRequired, User.adminRequired, Blog.new) //ok
  app.get('/admin/blog/update/:id', User.signinRequired, User.adminRequired, Blog.update) //ok
  app.post('/admin/blog', multipartMiddleware, User.signinRequired, User.adminRequired, Blog.save) //?
  app.get('/admin/blog/list', User.signinRequired, User.adminRequired, Blog.list) //ok
  app.delete('/admin/blog/list', User.signinRequired, User.adminRequired, Blog.del) //ok

  // 博客评论
  app.post('/user/blogComment', User.signinRequired, BlogComment.save)  //ok

  // 博客分类
  app.get('/admin/blog/blogCategory/new', User.signinRequired, User.adminRequired, BlogCategory.new) //ok
  app.post('/admin/blogCategory', User.signinRequired, User.adminRequired, BlogCategory.save) //ok
  app.get('/admin/blogCategory/list', User.signinRequired, User.adminRequired, BlogCategory.list)  //ok

  /*============== 电影网站路由 ==============*/
  // 电影主页路由
  app.get('/movie', MovieIndex.index)
